  ...
  try {
    ...
    try {
      ...
      await http.post(`${walletServiceUrl}/api/v1/wallets/debit`, {
        ...
        amount: amount,
        externalTransactionId: idempotencyKey // Usamos la idempotencyKey como ID externo
      });
      ...
      console.error('Wallet Service rechazó el débito:', debitError.response?.data || debitError.message);
      if (isNewRequest) await redisClient.del(key); 
      return res.status(debitError.response?.status || 400).send(debitError.response?.data || { message: "Error al contactar Wallet Service (Debit)" });
    }
    ...
    try {
      ...
      console.error('¡CRÍTICO! El débito se hizo, pero el crédito falló:', creditError.response?.data || creditError.message);
      ...
      try {
        ...
        console.log('Débito revertido (compensado).');
         ...
         console.error('¡¡¡FALLO CRÍTICO IRRECUPERABLE!!! El débito se hizo, el crédito falló y la compensación TAMBIÉN falló.');
         // Aquí se debe alertar a un administrador
      }

      if (isNewRequest) await redisClient.del(key);
      return res.status(creditError.response?.status || 500).send(creditError.response?.data || { message: "Error al contactar Wallet Service (Credit)" });
    }
    ...
    try {
      ...
      channel.sendToQueue(rmqQueue, Buffer.from(JSON.stringify(msg)), { persistent: true });
      await channel.close();
      await rmqConn.close();
    } catch (rmqErr) {
      console.error('Fallo al notificar a RabbitMQ (la transacción SÍ fue exitosa):', rmqErr.message);
    }

    // --- E. Actualizar Redis ---
    const finalResult = { transaction_id: transactionId, status: 'completed', amount, sender_wallet, receiver_wallet };
    ...
    res.status(500).send({ message: 'Error al procesar la transacción', error: error.message });
  } finally {
...
exports.findAllTransactions = async (req, res) => {
  try {
    const [rows] = await dbPool.query('SELECT * FROM Transaction ORDER BY timestamp DESC');
    res.status(200).send(rows);
  } catch (error) {
    res.status(500).send({ message: 'Error al leer transacciones', error: error.message });
  }
};
...
exports.findOneTransaction = async (req, res) => {
  const { id } = req.params;
  try {
    const [txRows] = await dbPool.query('SELECT * FROM Transaction WHERE Transaction_id = ?', [id]);
    
    if (txRows.length === 0) {
      return res.status(404).send({ message: 'Transacción no encontrada.' });
    }
    
    const [ledgerRows] = await dbPool.query('SELECT * FROM Ledger WHERE transaction_id = ?', [id]);
    ...
    res.status(500).send({ message: 'Error al leer transacción', error: error.message });
  }
};
...
exports.reverseTransaction = async (req, res) => {
    ...
    try {
        ...
        const [txRows] = await dbConnection.query('SELECT * FROM Transaction WHERE Transaction_id = ?', [id]);
        if (txRows.length === 0) {
            return res.status(404).send({ message: 'Transacción original no encontrada.' });
        }
        
        const originalTx = txRows[0];

        if (originalTx.status === 'reversed') {
            return res.status(400).send({ message: 'Esta transacción ya fue revertida.' });
        }
        if (originalTx.status !== 'completed') {
            return res.status(400).send({ message: 'Solo se pueden revertir transacciones completadas.' });
        }
        ...
        res.status(201).send({ message: 'Transacción revertida exitosamente', new_transaction_id: reverseTxId });

    } catch (error) {
        if (dbConnection) await dbConnection.rollback();
        console.error('Error al revertir transacción:', error.message);
        res.status(500).send({ message: 'Error al revertir transacción', error: error.message });
    } finally {
        if (dbConnection) dbConnection.release();
    }
};